...
class GeckoSessionWrapper(
    ...
    private fun createSession(): GeckoSession {
        return try {
            val sb = GeckoSessionSettings.Builder().usePrivateMode(false).allowJavascript(true)
    ...
    fun open() {
        ...
        val ctx = appContext ?: run { Log.w(TAG, "open: null ctx"); return }
        if (Looper.myLooper() != Looper.getMainLooper()) { mainHandler.post { open() }; return }
        try {
            val rt = GeckoRuntimeManager.getOrInit(ctx)
            if (rt == null) { Log.w(TAG, "runtime not ready — retry 400ms"); mainHandler.postDelayed({ open() }, 400); return }
            session.open(rt)
            Log.d(TAG, "session opened contextId=$contextId")
        } catch (e: Exception) { Log.e(TAG, "open failed: ${e.message}", e) }
    }

    fun close() { try { if (session.isOpen) session.close() } catch (e: Exception) { Log.w(TAG, "close: ${e.message}") } }

    fun loadUrl(url: String) {
        if (!session.isOpen) {
            open()
            // Schedule the load after open() has dispatched to Main and executed
            mainHandler.postDelayed({ safeLoad(url) }, 500)
        } else safeLoad(url)
    }

    private fun safeLoad(url: String) { try { session.loadUri(url) } catch (e: Exception) { Log.e(TAG, "loadUri: ${e.message}") } }

    fun goBack() { try { session.goBack() } catch (e: Exception) { Log.w(TAG, "back: ${e.message}") } }
    fun goForward() { try { session.goForward() } catch (e: Exception) { Log.w(TAG, "fwd: ${e.message}") } }
    ...
    private fun buildContentDelegate() = object : ContentDelegate {
        ...
        override fun onFullScreen(session: GeckoSession, fullScreen: Boolean) { try { onFullscreenChange?.invoke(fullScreen) } catch (e: Exception) { Log.w(TAG, "fs: ${e.message}") } }
        override fun onExternalResponse(session: GeckoSession, response: WebResponse) {
            try {
                ...
                if (appContext != null && downloadHandler != null) {
                    permissionHandler?.requestDownloadConfirmation(url, filename, mime, size) ?: downloadHandler.enqueueDownload(appContext, url, filename, mime, size)
                }
